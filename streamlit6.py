...
def add_embedding_to_db(document: str, embedding, source: str = "user", extra_metadata: dict = None):
    ...
    Note: We do not show success message here for 'crawl' to avoid overshadowing 
    the final success message in the 'crawl_and_embed' function.
    """
    try:
        ...
        # Show a success message only if this was a user prompt, not a crawl
        if source == "user":
            st.success("Document from user added to the database.")
...
def stream_llm_response(prompt: str):
    ...
    We parse out the JSON, and then yield choices[0]["text"] 
    to display a readable incremental response.
    """
    payload = {
        "model": LLM_MODEL,
        "prompt": prompt,
        ...
        with requests.post(OLLAMA_LLM_ENDPOINT, json=payload, stream=True) as response:
            ...
            for line in response.iter_lines():
                ...
                if not line_str.startswith("data: "):
                    # If there's something else, skip
                    continue
...
def crawl_and_embed(url: str):
    ...
    The success message about website embedding completion 
    is shown only if all steps succeed.
    """
    ...
    # 4. Final success message, shown only if all above steps succeed
...
def main():
    ...
    if st.button("Submit Query") and prompt:
        ...
        # 4. Build augmented prompt and stream the LLM response
