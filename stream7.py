...
# Define static instructions for the LLM.
STATIC_INSTRUCTIONS = (
    "All your answers are specifically about A.R.Rahman who is a noted music composer, and about no other topics."
    "look for information about A.R.Rahman on his wikipedia article which you have access to, and address questions accordingly"
)
...
def add_embedding_to_db(document: str, embedding, source: str = "user", extra_metadata: dict = None):
    ...
    For user documents, a success message is displayed immediately.
    For crawled content, the success message is shown only after all steps complete.
    """
    try:
...
def build_augmented_prompt(user_prompt: str, context_docs: list) -> str:
    """
    Combines static instructions, any retrieved context, and the user prompt into an augmented prompt.
    """
    ...
    if context_docs:
        try:
            ...
            f"Query: {user_prompt}"
             ...
             st.error(f"Error retrieving context from ChromaDB. Review and rerun agent: {e}")
...
def stream_llm_response(prompt: str):
    ...
    The function parses the JSON and yields choices[0]["text"] for a readable response.
    """
    payload = {
        "model": LLM_MODEL,
        "prompt": prompt,
...
def crawl_and_embed(url: str):
    ...
    The success message is shown only after all steps complete.
    """
...
def main():
    ...
    if st.button("Submit Query") and prompt:
        ...
        # Build the augmented prompt with static instructions
